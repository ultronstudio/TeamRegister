        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="p-5">
                    ...
                    @endif
                    <div>
                        <p style="color: var(--bs-black);">Krok <b>{{ $krokAktualni }}</b> ze <b>{{ $krokyCelkem }}</b>
                        </p>
                        <div class="progress mb-3">
                            <div class="progress-bar" id="progressbar" role="progressbar"
                                aria-label="Stav kompletace formuláře"
                                style="width: {{ ($krokAktualni / $krokyCelkem) * 100 }}%"
                                aria-valuenow="{{ ($krokAktualni / $krokyCelkem) * 100 }}" aria-valuemin="0"
                    ...
                    <form class="user">
                            ...
                            <div class="row mb-3">
                                <div class="col-sm-6 mb-3 mb-sm-0">
                                    <input wire:model="nazev" class="form-control form-control-user" type="text"
                                        id="exampleFirstName-1" placeholder="Název týmu" name="nazev"
                                        title="Název týmu" @if (!empty($existingTeams)) list="tymy" @endif
                                        autocomplete="off">
                                    @error('nazev')
                                ...
                                <div class="col-sm-6">
                                    <input wire:model="pocet" wire:keydown="upravPocet"
                                        ...
                                        placeholder="Počet členů" name="pocet" title="Počet členů v týmu"
                                        min="1">
                                    @error('pocet')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col">
                                        <input wire:model="soutezici.{{ $klic }}.kategorie"
                                            ...
                                            aria-describedby="clenKategorie" placeholder="Kategorie"
                                            name="clenKategorie" title="Kategorie soutěžícího podle věku" readonly>
                                    ...
                                    @if (count($soutezici) > 1)
                                        <div class="col">
                                            <button wire:click="smazatSouteziciho({{ $klic }})"
                                                class="btn btn-danger d-block btn-user w-100 h-100"
                                                type="button">Odstranit
                                                soutěžícího</button>
                                        </div>
                                    @endif
                                </div>
                        ...
                        <div class="row">
                            @if ($krokAktualni > 1)
                                <div class="col">
                                    <button wire:click="predchoziKrok" class="btn btn-primary d-block btn-user w-100"
                                        style="color: var(--bs-white); background: var(--bs-blue);border-style: none;"
                                        type="button">Předchozí</button>
                                </div>
                            @endif
                            @if ($krokAktualni < $krokyCelkem)
                                <div class="col">
                                    <button wire:click="dalsiKrok" class="btn btn-primary d-block btn-user w-100"
                                        style="color: var(--bs-white); background: var(--bs-blue);border-style: none;"
                                        type="button">Další</button>
                                </div>
                            @endif
                            @if ($krokAktualni == $krokyCelkem)
                                <div class="col">
                                    <button wire:click="ulozit" class="btn btn-primary d-block btn-user w-100"
                                        style="color: var(--bs-white); background: var(--bs-green);border-style: none;"
                                        type="button">Registrovat</button>
                                </div>
                            @endif
                        </div>
                    </form>
